...
export function LiveFeed() {
  ...
  return (
    <div
      className={`flex items-center gap-2.5 transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`}
      aria-live="polite"
    ...
    >
      ...
      <p className="text-sm text-white/70 whitespace-nowrap overflow-hidden text-ellipsis">
        ...
        {' '}from{' '}
        <span className="text-white font-semibold">{entry.city}</span>
        {' '}just staked{' '}
        <span className="text-emerald-400 font-semibold">{entry.amount} {entry.asset}</span>
        {' '}·{' '}
        <span className="text-white/40">{minutesAgo} min ago</span>
      </p>
    </div>
  )
}
