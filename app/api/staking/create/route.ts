...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`staking:${ip ?? 'unknown'}`, 10, 60 * 60_000)
  ...
  try {
    ...
    if (!userRl.success) return rateLimitResponse(userRl.retryAfter)
    ...
    if (plan.name === 'Starter Trial') {
      ...
      if (alreadyUsed) {
        return NextResponse.json(
          { error: 'The Starter Trial plan can only be used once. Please choose another plan.' },
          { status: 400 }
        )
      }
    }
